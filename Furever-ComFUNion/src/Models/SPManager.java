...
public class SPManager {
    ...
    private ArrayList<Application> getApplicationHistoryForVetByType(String vetID, String applicationType) {
        ...
        String query = "SELECT a.ApplicationID, a.ApplicationType, a.AppointDate, a.AppointTime, a.AppointStatus, "
    ...
    public ArrayList<List<String>> buildAndExecuteSummaryQuery(List<String> displayAttributes, List<String> columnFunctions, List<String> havingConditions) {
        ...
        try (Connection connection = DriverManager.getConnection(URL, USERNAME, PASSWORD);
            ...
            for (String function : columnFunctions) {
                String[] parts = function.split(" AS ");
                    if (parts.length > 1) {
                        header.add(parts[1]);
                    }
            }
            ...
            while (resultSet.next()) {
                ...
                for (String attribute : displayAttributes) {
                    row.add(resultSet.getString(attribute));
                }
                for (String function : columnFunctions) {
                    String[] parts = function.split(" AS ");
                    if (parts.length > 1) {
                        row.add(resultSet.getString(parts[1]));
                    }
                }
                results.add(row);
                System.out.println();
    ...
    private String buildQuery(List<String> displayAttributes, List<String> columnFunctions, List<String> havingConditions) {
        ...
        // Check for required tables based on attributes
        for (String attribute : displayAttributes) {
            if (attribute.contains("Pet")) {
                tables.add("forevercomfunion.`pet.v2` p");
            }
            if (attribute.contains("Vet")) {
                tables.add("forevercomfunion.`vet.v2` v");
            }
            if (attribute.contains("Client")) {
                tables.add("forevercomfunion.`client.v2` c");
            }
            if (attribute.contains("App")) {
                tables.add("forevercomfunion.`application.v2` a");
            }
        }
        for (String function : columnFunctions) {
            if (function.contains("Pet")) {
                tables.add("forevercomfunion.`pet.v2` p");
            }
            if (function.contains("Vet")) {
                tables.add("forevercomfunion.`vet.v2` v");
            }
            if (function.contains("Client")) {
                tables.add("forevercomfunion.`client.v2` c");
            }
            if (function.contains("App")) {
                tables.add("forevercomfunion.`application.v2` a");
            }
        }

        // Add the application table if pet, vet, or client is present
        boolean requiresApplication = tables.contains("forevercomfunion.`pet.v2` p") &&
                                      tables.contains("forevercomfunion.`vet.v2` v") ||
                                      tables.contains("forevercomfunion.`pet.v2` p") &&
                                      tables.contains("forevercomfunion.`client.v2` c") ||
                                      tables.contains("forevercomfunion.`vet.v2` v") &&
                                      tables.contains("forevercomfunion.`client.v2 c");
        ...
        // Add FROM clause based on required tables
        queryBuilder.append(" FROM ");
        queryBuilder.append(String.join(", ", tables)).append(" ");

        // Add WHERE clause based on included tables if more than one table is included
        if (tables.size() > 1) {
            queryBuilder.append("WHERE ");
            List<String> conditions = new ArrayList<>();

            boolean hasApplication = tables.contains("forevercomfunion.`application.v2` a");
            boolean hasClient = tables.contains("forevercomfunion.`client.v2` c");
            boolean hasPet = tables.contains("forevercomfunion.`pet.v2` p");
            boolean hasVet = tables.contains("forevercomfunion.`vet.v2` v");

            if (hasApplication) {
                if (hasClient) {
                    conditions.add("a.ClientID = c.ClientID");
                }
                if (hasPet) {
                    conditions.add("a.PetID = p.PetID");
                }
                if (hasVet) {
                    conditions.add("a.VetID = v.VetID");
                }
            }

            if (conditions.isEmpty()) {
                // Handle the case where there are no valid conditions
                throw new IllegalArgumentException("Invalid table combination for WHERE clause.");
            }
